...
public:
	...
	virtual void onSessionJoin()
	{
		...
		if (enableCommandChannel) {
			...
			zchar_t* tempCmd = "hello world";
			
			ZoomVideoSDKErrors err = commandChannel->sendCommand(NULL, tempCmd);
	...
	virtual void onCommandChannelConnectResult(bool isSuccess) {
		...
		if (enableCommandChannel) {
			...
			if (isSuccess ==true) {
			printf("onCommandChannelConnectResult() Message: Success\n");


			IZoomVideoSDKCmdChannel* commandChannel = video_sdk_obj->getCmdChannel();

			zchar_t* tempCmd= "I've connected to command channel";
			ZoomVideoSDKErrors err = commandChannel->sendCommand(NULL, tempCmd);
			...
			printf("onCommandChannelConnectResult() Message: Failure\n");
		}
	}
