...
DeepgramClient deepgram(apiKey);
...
virtual void onMixedAudioRawDataReceived(AudioRawData* data_) {
		if (getRawAudio) {
			...
			if (data_) {
				...
				if (counter >= 500){
					//print message which says it is more than 40
					std::cout << "Counter is more than 500" << std::endl;

					counter = 0;

					// Send the audio file and get the result
					if (deepgram.sendAudioFile(audioFile)) {
						std::cout << "Audio file successfully sent to Deepgram API." << std::endl;
					}
					else {
						std::cerr << "Failed to send audio file to Deepgram API." << std::endl;
					}
